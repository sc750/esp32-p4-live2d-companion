...
lv_display_t *lvgl_adapter_init(const bsp_display_config_t *cfg)
{
    ...
    if (err != ESP_OK) {
        /* The 1024x600 Function EV LCD can be used without a GT911
         * touch controller.  Do not abort display startup when touch is
         * absent or its optional I2C device is not responding. */
